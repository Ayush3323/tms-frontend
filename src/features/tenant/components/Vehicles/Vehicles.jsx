...
import { useVehicles, useUpdateVehicle, useCreateVehicle } from '../../queries/vehicles/vehicleQuery';
        ...
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden">
          ...
          <ul className="max-h-48 overflow-y-auto divide-y divide-gray-50">
            ...
            {!isLoading && types.length === 0 && (
              <li className="px-4 py-3 text-xs text-gray-400 text-center">No types found</li>
            )}
            {/* Clear option */}
            {value && (
              <li
                onClick={() => { onChange(''); setOpen(false); setQuery(''); }}
                className="px-4 py-2 cursor-pointer hover:bg-red-50 text-xs text-red-400 transition-colors"
              >
                ✕ Clear selection
              </li>
            )}
            {types.map(t => (
              <li
                ...
                className={`px-4 py-2.5 cursor-pointer hover:bg-blue-50 transition-colors
                  ...
                  ${t.id === value ? 'bg-blue-50' : ''}`}
              >
                <span className="font-semibold text-[#172B4D] text-sm">{t.name}</span>
                {t.description && (
...
const AddVehicleModal = ({ onClose }) => {
  ...
  const handleSubmit = () => {
    createVehicle.mutate(form, { onSuccess: () => onClose() });
  };

  const canSubmit = form.registration_number && form.make && form.model
    && form.fuel_type && form.ownership_type && !createVehicle.isPending;
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-black text-[#172B4D]">Add New Vehicle</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input placeholder="e.g. Ashok Leyland" value={form.make} onChange={set('make')} />
            </div>
            <div>
              <Label required>Model</Label>
              <Input placeholder="e.g. Dost" value={form.model} onChange={set('model')} />
            </div>
            <div>
              <Label>Year</Label>
            ...
            <div>
              <Label>Color</Label>
              <Input placeholder="e.g. Blue" value={form.color} onChange={set('color')} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label>Capacity (Tonnage)</Label>
              <Input type="number" placeholder="e.g. 15" value={form.capacity_tonnage} onChange={set('capacity_tonnage')} />
            </div>
            <div>
              <Label>Capacity (Volume m³)</Label>
              <Input type="number" placeholder="e.g. 30" value={form.capacity_volume} onChange={set('capacity_volume')} />
            </div>
            <div>
              <Label>Current Odometer (km)</Label>
              <Input type="number" placeholder="0" value={form.current_odometer} onChange={set('current_odometer')} />
            </div>

            {/* ✅ Vehicle Type — searchable dropdown instead of UUID input */}
            <div>
              <Label>Vehicle Type</Label>
              <VehicleTypeSelect
                value={form.vehicle_type}
                onChange={(id) => setForm(p => ({ ...p, vehicle_type: id }))}
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input type="number" placeholder="e.g. 1800000" value={form.purchase_price} onChange={set('purchase_price')} />
            </div>
          </div>
        </div>
...
const Vehicles = () => {
  ...
  const COLUMNS = [
    ...
    {
      header: 'Year',
      render: v => <span className="text-gray-600">{v.year ?? '—'}</span>,
    },
    {
      header: 'Fuel / Trans',
      render: v => (
        <div className="flex flex-col gap-1">
          ...
          {v.transmission_type &&
            <span className="text-[11px] text-gray-400">{v.transmission_type}</span>
          }
        </div>
      ),
    },
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {COLUMNS.map(c => (
                    <th key={c.header} className="text-left px-4 py-3 text-[11px] font-bold text-gray-400 uppercase tracking-widest whitespace-nowrap">
                      {c.header}
                    </th>
                  ))}
                </tr>
